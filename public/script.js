...
const getArticles = async (qiita, zenn, issou, str) => {
  ...
  const response = await fetch(
    `/article?qiita=${qiita}&zenn=${zenn}&issou=${issou}${
      (str !== "") ? `&q=${str}` : ""
    }`,
  );
...
const getArticleHTMLElement = () => {
  ...
  const comments = article.comments_count === undefined
    ? 0
...
globalThis.onload = async () => {
  ...
  openPopupBtn.addEventListener("click", () => {
    const x = document.getElementById(
      "popup-wrapper",
    ); /*クラス名"popup-wrapper"のオブジェクトの配列を取得*/
...
setInterval(async () => {
  ...
  if (articles.length <= 5) {
    ...
    articles.unshift(
      ...shuffleArray(await getArticles(qiita, zenn, issou, searchInputValue)),
    );
  }
}, 7000);
